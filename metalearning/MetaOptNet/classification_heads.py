...
def MetaOptNetHead_Ridge(query, support, support_labels, n_way, n_shot, lambda_reg=50.0, double_precision=False):
    ...
    if torch.cuda.is_available():
        dummy = dummy.cuda()      # We want to ignore the equality constraint.

    if double_precision:

        G, e, C, h = [x.double().cuda() if torch.cuda.is_available() else x.double() for x in [G, e, C, h]]

    else:
        G, e, C, h = [x.float().cuda() if torch.cuda.is_available() else x.float() for x in [G, e, C, h]]
...
def MetaOptNetHead_SVM_He(query, support, support_labels, n_way, n_shot, C_reg=0.01, double_precision=False):
    ...
    kernel_matrix = computeGramMatrix(support, support)
    t_ones = torch.ones(tasks_per_batch, n_support, n_way)
    if torch.cuda.is_available():
        t_ones = t_ones.cuda()
    V = (support_labels * n_way - t_ones) / (n_way - 1)
    ...
    dummy = Variable(torch.Tensor())     # We want to ignore the equality constraint.
    if torch.cuda.is_available():
        dummy = dummy.cuda()
    ...
    if double_precision:
        ...
        G, e, C, h = [x.cuda() if torch.cuda.is_available else x for x in [G, e, C, h]]
...
def MetaOptNetHead_SVM_CS(query, support, support_labels, n_way, n_shot, C_reg=0.1, double_precision=False, maxIter=15):
    ...
    tmp = torch.eye(n_way*n_support).expand(tasks_per_batch, n_way*n_support, n_way*n_support)
    if torch.cuda.is_available():
        tmp = tmp.cuda()
    block_kernel_matrix += 1.0 * tmp
    ...
    if torch.cuda.is_available():
        id_matrix_2 = id_matrix_2.cuda()

    t_ones = torch.ones(tasks_per_batch, 1, n_way)
    if torch.cuda.is_available():
        t_ones = t_ones.cuda()
    A = Variable(batched_kronecker(id_matrix_2, t_ones))
    b = Variable(torch.zeros(tasks_per_batch, n_support))
    #print (A.size(), b.size())
    if double_precision:
        G, e, C, h, A, b = [x.double().cuda() if torch.cuda.is_available() else x.double() for  x in [G, e, C, h, A, b]]
    else:
        G, e, C, h, A, b = [x.float().cuda() if torch.cuda.is_available() else x.float() for x in [G, e, C, h, A, b]]
...
def MetaOptNetHead_SVM_WW(query, support, support_labels, n_way, n_shot, C_reg=0.00001, double_precision=False):
    ...
    kernel_matrix = computeGramMatrix(support, support) + torch.ones(tasks_per_batch, n_support, n_support)
    if torch.cuda.is_available():
        kernel_matrix = kernel_matrix.cuda()
    ...
    kernel_matrix_mask_second_term = kernel_matrix_mask_second_term == torch.arange(n_way).long().repeat(n_support).reshape(n_support, n_way).transpose(1, 0).reshape(1, -1).repeat(n_support, 1)
    if torch.cuda.is_available():
        kernel_matrix_mask_second_term = kernel_matrix_mask_second_term.cuda()
    ...
    t_ones = torch.ones(tasks_per_batch, n_way * n_support)
    if torch.cuda.is_available():
        t_ones = t_ones.cuda()
    C_mat = C_reg * t_ones - C_reg * Y_support

    C = Variable(torch.cat((id_matrix, -id_matrix), 1))
    #C = Variable(torch.cat((id_matrix_masked, -id_matrix_masked), 1))
    zer = torch.zeros(tasks_per_batch, n_way * n_support)
    if torch.cuda.is_available():
        zer = zer.cuda()
    ...
    # Compute the classification score.
    t_ones = torch.ones(tasks_per_batch, n_support, n_query).cuda()
    if torch.cuda.is_available():
        t_ones = t_ones.cuda()
    compatibility = computeGramMatrix(support, query) + t_ones
    compatibility = compatibility.float()
