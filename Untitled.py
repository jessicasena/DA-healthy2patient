...
def read_annotation_file():
    ann = pd.read_csv("Activity_Annotation_Agreement_2019-11-23.csv")
    ...
    for i in range(um_acc.shape[0]):
        ...
        if right_ts == left_ts:
            if (right_ts - delta) < acc_timestamp < (right_ts + delta):
                labels_voting = multimode(timestamp_labels[right_ts])
            ...
            matched = True
        if not matched:
            distance = abs(acc_timestamp-left_ts) if abs(acc_timestamp-left_ts) < abs(acc_timestamp-right_ts) else abs(acc_timestamp-right_ts)
            um_acc[i, -1] = -1
...
for file in accs_files.keys():
    ...
    final_acc_file = get_labels(timestamp_labels, um_acc)

    not_matched = np.count_nonzero(final_acc_file[:,-1] == -1)
    print(f"Not Matched: {not_matched}, {(not_matched/len(final_acc_file))*100:.3}%")
