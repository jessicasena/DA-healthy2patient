    ...
    def call(self, inputs):
        ...
        if self.alignment_type == 'global':                                                         # Global Approach
            ...
            if self.alignment_type == 'local-m':                                                    # Monotonic Alignment
                ...
                onehot_vector = tf.reduce_sum(input_tensor=onehot_vector, axis=1)                                # (B, S)
        ...
        if 'dot' in self.score_function:                                                            # Dot Score Function
            ...
            attention_score = tf.reduce_sum(input_tensor=attention_score, axis=-1)                               # (B, S*)
...
class SelfAttention(Layer):
    ...
    def call(self, inputs):  # (B, S, H)
        ...
        if self.use_penalization:
            ...
            frobenius_norm = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(product - identity)))  # distance
